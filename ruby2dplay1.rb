...
two_pi = 2 * Math::PI
angle = 0
angle_incr_mult = 0.001
angle_incr = angle_incr_mult * Math::PI / 180
x_sign = 1
y_sign = 1
...
update do
  lines.each do |line|
    ...
    (line.x1 + x_move).between?(0, 640) ? x_sign = 1 : x_sign = -1
    (line.y1 + y_move).between?(0, 480) ? y_sign = 1 : y_sign = -1
